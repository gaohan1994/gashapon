export const SHOW_SEARCH_MODAL = 'SHOW_SEARCH_MODAL';
export type SHOW_SEARCH_MODAL = typeof SHOW_SEARCH_MODAL;

export const HIDE_SEARCH_MODAL = 'HIDE_SEARCH_MODAL';
export type HIDE_SEARCH_MODAL = typeof HIDE_SEARCH_MODAL;

export const ADD_SEARCH_ITEM = 'ADD_SEARCH_ITEM';
export type ADD_SEARCH_ITEM = typeof ADD_SEARCH_ITEM;

export const EMPTY_SEARCH_ITEMS = 'EMPTY_SEARCH_ITEMS';
export type EMPTY_SEARCH_ITEMS = typeof EMPTY_SEARCH_ITEMS;

export const SHOW_NEWS = 'SHOW_NEWS';
export type SHOW_NEWS = typeof SHOW_NEWS;

export const HIDE_NEWS  = 'HIDE_NEWS';
export type HIDE_NEWS = typeof HIDE_NEWS;

export const SHOW_CHANGE_PHONE = 'SHOW_CHANGE_PHONE';
export type SHOW_CHANGE_PHONE = typeof SHOW_CHANGE_PHONE;

export const HIDE_CHANGE_PHONE = 'HIDE_CHANGE_PHONE';
export type HIDE_CHANGE_PHONE = typeof HIDE_CHANGE_PHONE;

export const RECEIVE_NOTIFIES = 'RECEIVE_NOTIFIES';
export type RECEIVE_NOTIFIES = typeof RECEIVE_NOTIFIES;

export const SHOW_LOGIN = 'SHOW_LOGIN';
export type SHOW_LOGIN = typeof SHOW_LOGIN;

export const HIDE_LOGIN = 'HIDE_LOGIN';
export type HIDE_LOGIN = typeof HIDE_LOGIN;

export const SHOW_REGISTER = 'SHOW_REGISTER';
export type SHOW_REGISTER = typeof SHOW_REGISTER;

export const HIDE_REGISTER = 'HIDE_REGISTER';
export type HIDE_REGISTER = typeof HIDE_REGISTER;

export const SHOW_LOGIN_MODAL = 'SHOW_LOGIN_MODAL';
export type SHOW_LOGIN_MODAL = typeof SHOW_LOGIN_MODAL;

export const HIDE_LOGIN_MODAL = 'HIDE_LOGIN_MODAL';
export type HIDE_LOGIN_MODAL = typeof HIDE_LOGIN_MODAL;

export const SHOW_SIGN_MODAL = 'SHOW_SIGN_MODAL';
export type SHOW_SIGN_MODAL = typeof SHOW_SIGN_MODAL;

export const HIDE_SIGN_MODAL = 'HIDE_SIGN_MODAL';
export type HIDE_SIGN_MODAL = typeof HIDE_SIGN_MODAL;

export const SET_ORDER_ADDRESS_CONFIG = 'SET_ORDER_ADDRESS_CONFIG';
export type SET_ORDER_ADDRESS_CONFIG = typeof SET_ORDER_ADDRESS_CONFIG;

export const SHOW_SHARE_MODAL = 'SHOW_SHARE_MODAL';
export type SHOW_SHARE_MODAL = typeof SHOW_SHARE_MODAL;

export const HIDE_SHARE_MODAL = 'HIDE_SHARE_MODAL';
export type HIDE_SHARE_MODAL = typeof HIDE_SHARE_MODAL;